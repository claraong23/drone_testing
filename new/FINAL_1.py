...
def on_detection(detections, annotated_image, context):
    """Detector callback: log barrel sightings."""
    for d in detections:
        print(f"\n{'✅'}")
...
def image_callback(msg: Image):
    """Gazebo RGB callback: submit each frame to YOLO."""
    if detector is None:
        return
...
class RightWallFollower:
    ...
    def __init__(self, depth_topic="/depth_camera"):
        ...
        self.first_opening_scan_done = False
    ...
    # ------------------------------------------------------------------
    # Right-opening detection (persistence counter, from try2.py)
    # ------------------------------------------------------------------
    ...
    async def scan_left_and_return(self):
        """Nudge back, turn 90° left, pause, move forward to original spot, turn back, resume."""
        print("[SCAN] Scanning left before corner follow")

        NUDGE_M = 0.8          # how far to nudge back/forward
        NUDGE_SPEED = 0.4      # m/s
        nudge_duration = NUDGE_M / NUDGE_SPEED  # 0.75s

        # Move back
        steps = max(1, int(nudge_duration / self.CONTROL_DT_S))
        for _ in range(steps):
            await self.set_body_velocity(-NUDGE_SPEED, 0.0, 0.0, 0.0)
            await asyncio.sleep(self.CONTROL_DT_S)
        await self.hold_position(0.2)

        # Turn left 90°
        original_yaw = await self.get_current_yaw_deg()
        target_left = ((original_yaw - 90.0) + 180.0) % 360.0 - 180.0
        await self.rotate_to_yaw(target_left)
        await self.hold_position(1.0)  # pause for YOLO to scan

        # Turn back to original heading
        await self.rotate_to_yaw(original_yaw)
        await self.hold_position(0.2)

        # Move forward to revert position
        for _ in range(steps):
            await self.set_body_velocity(NUDGE_SPEED, 0.0, 0.0, 0.0)
            await asyncio.sleep(self.CONTROL_DT_S)
        await self.hold_position(0.2)

        print("[SCAN] Scan done, resuming")
    ...
    async def task_loop(self):
        ...
        while self.running:
            ...
            if opening or (front_blocked and right_is_open):
                ...
                if not self.first_opening_scan_done:
                    await self.scan_left_and_return()
                    self.first_opening_scan_done = True
