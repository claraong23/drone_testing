#!/usr/bin/env python3
"""
right wall following for nav, barrel detection with my_model.pt, and a live feed display

how the code works
- follows the wall on the right using depth data and a proportional yaw controller
- if an open space is detected on the right it rounds the corner to explore 
  (simply turning 90 degrees is not enough for the drone to turn reliably without hitting the wall so need to trace the corner)
- once its done rounding the corner, it will snap to the nearest cardinal heading 
  (to ensure the drone is parallel to the new corridor and not angled into the wall or it will bang into it)
- uses the new my_model.pt instead of roboverse.pt (old version)
- a live feed will pop up so you can see what the drone sees
- whenever a barrel is detected it will print out the bounding box frame for a short time before going back to the raw feed
- all detections are saved in a detections folder
"""
import asyncio
import sys
import threading
...
class RightWallFollower:
    ...
    RIGHT_FOLLOW_DISTANCE_M = 1.5         # standoff held from the right wall
    ...
    # ------------------------------------------------------------------
    # Right-opening detection (persistence counter)
    # ------------------------------------------------------------------
